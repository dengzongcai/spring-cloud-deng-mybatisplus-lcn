...
public class StringUtil {
    ...
    public static String removeBrackets(String args){
        args = args.replaceAll("\\(","").replaceAll("\\)","").replaceAll("（","").replaceAll("）","");
        return args;
    }

	public static boolean isNullOrEmptyVar(String... args) {
		boolean flag = false;
    ...
    public static boolean hasNullOrEmpty(String...args) {
    	...
    	if(args==null||args.length<1){
    	  ...
    	  for(int i=0;i<args.length;i++){
    		  if (args[i] == null || args[i].length() == 0||"null".equals(args[i])){
    			  return true;
    		  }
    	  }
    	}
    ...
    public static String moneyToString(Object money) {
        if (money != null) {
            DecimalFormat df = new DecimalFormat("#.##");
            return df.format((Double) money);
        }
        return null;
    }
    ...
    public static Map<String, String> parseQuery(String query, char split1,
        ...
        if (!isNullOrEmpty(query) && query.indexOf(split2) > 0) {
    ...
    public static String regexEscap(String s) {
