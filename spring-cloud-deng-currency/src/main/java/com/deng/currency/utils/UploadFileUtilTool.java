...
import com.deng.commons.utils.VideoScreensShotUtil;
...
//		private final static Lock aLock=new ReentrantLock();
    ...
    public static Map<String, Object> upload(MultipartFile file, String prjName,
                                             String basePath, String format  ) throws Exception {
        ...
        String panfu = ResourceUtil.getConfigByName("video.panfuurl");
        ...
        String[] strarr = null;
        if (filepath != null) {
            strarr = filepath.split(ResourceUtil.getConfigByName("video.panfuurl"));
        }
        String preName = "";// 后缀、文件名
        if (strarr != null) {
            preName = strarr[1];
        }
        preName = filepath.substring(filepath.indexOf(ResourceUtil.getConfigByName("video.panfuurl")));
        ...
        if (!file.isEmpty()) {
            try {
                //File.separator
                final String vpath=filepath  + name;
                String getshotimg=filenameNoext+1+".jpg";
                //文件真实路径
                realpath=vpath;
                logger.info("文件真实路径："+vpath);
                final File file_in= new File(vpath);
                logger.info("文件："+file_in);
